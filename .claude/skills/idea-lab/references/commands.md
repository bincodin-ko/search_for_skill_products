...
## lint (보완 #15)
`LAB lint <파일> [<파일> ...] [--threshold 0.45]` — 발굴 파일을 import하기 전에 점검: ① 옛 카드와 개념 중복(제목 글자쌍, 괄호 부연 제외) ② 통과·보류 줄의 필수 칸(signal_url·pay_signal·payer·gap_signal·market·prefilter + 지불 근거) 누락 ③ 기법·시장·분야 분포(여러 파일을 주면 누적). 중복 10% 또는 누락 20% 초과면 종료 코드 1
