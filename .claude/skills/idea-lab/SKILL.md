...
- **규칙이 의견보다 우선한다.** need ≤ 2 또는 수익성(max(revenue, fame)) ≤ 2면 Drop. **need는 고통 또는 욕망**(욕망은 행동 증거로), **pain 관문은 원문 대신 행동 증거도 인정**, **비싸거나 불만 많은 유료 경쟁은 이길 틈(beatable)**, **해외 시장도 본다**(`market` global/both, 대시보드 🌍), 1인 운영 부담(`ops`)·수익 모델 종류(`revenue_math.model`)·왜 지금(`why_now`, 증거가 얇으면 `wait` → 타이밍 대기)을 함께 본다. **수익성은 두 길**: 유료 고객으로 월 300만 원(revenue) 또는 무료라도 유명해져 광고·파생 제품·제휴·인수로 돈이 되는 길(fame, 기본 목표 1년 월 사용자 1만 명). 예외 후보를 살릴지는 사용자가 정한다
